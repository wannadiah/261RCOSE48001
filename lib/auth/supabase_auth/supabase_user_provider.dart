...
class StepQuestSupabaseUser extends BaseAuthUser {
  StepQuestSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> stepQuestSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = StepQuestSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
